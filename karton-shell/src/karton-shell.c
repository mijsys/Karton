...
struct wl_output *output;
struct wl_output *outputs[MAX_WL_OUTPUTS];
struct output_state output_states[MAX_WL_OUTPUTS];
size_t output_count;
struct wl_seat *seat;
...
bool quick_open;
uint64_t top_popup_open_started_ms;
int top_popup_mode;
bool dnd_enabled;
...
bool launcher_open;
uint64_t launcher_open_started_ms;
...
struct panel desktop_extra[MAX_WL_OUTPUTS - 1];
size_t desktop_extra_count;

char desktop_dir[PATH_MAX];
struct timespec desktop_dir_mtime;
bool desktop_dir_mtime_valid;
uint64_t desktop_last_scan_ms;
...
bool desktop_positions_need_migration;
...
int desktop_menu_target;
int desktop_menu_hover;
int desktop_menu_panel_slot;
...
static char clock_timezone_candidates[256][64];
static size_t clock_timezone_candidate_count = 0;

static void
clock_load_available_timezones(void)
{
FILE *f = fopen("/usr/share/zoneinfo/zone.tab", "r");
if (!f) return;

clock_timezone_candidate_count = 0;
// Always add UTC as first explicit option
snprintf(clock_timezone_candidates[clock_timezone_candidate_count++], 64, "UTC");

char line[256];
while (fgets(line, sizeof(line), f) && clock_timezone_candidate_count < 256) {
if (line[0] == '#') continue;
char *country = strtok(line, "\t");
char *coords = strtok(NULL, "\t");
char *zone = strtok(NULL, "\t\n");
if (country && coords && zone && strchr(zone, '/')) {
snprintf(clock_timezone_candidates[clock_timezone_candidate_count++], 64, "%s", zone);
}
}
fclose(f);
}
...
static void draw_desktop_panels(struct app *app);
static void update_desktop_input_region(struct panel *panel);
...
static bool desktop_refresh_if_needed(struct app *app);
static struct panel *desktop_panel_from_surface(struct app *app, struct wl_surface *surface);
static int desktop_panel_slot(const struct app *app, const struct panel *panel);
static const char *desktop_output_name_from_slot(const struct app *app, int slot);
static int desktop_output_slot_from_name(const struct app *app, const char *name);
static void desktop_item_assign_output(struct app *app, struct desktop_item *item,
int slot, const char *output_name);
static const char *desktop_icon_fallback_name(const char *path, const char *content_type, bool executable);
static void desktop_item_resolve_icon(struct desktop_item *item);
static bool desktop_launch_in_terminal(const char *command, const char *cwd, GError **error);
static bool desktop_menu_rect(const struct app *app, const struct panel *panel,
double *x, double *y, double *w, double *h);
static int desktop_menu_item_hit(const struct app *app, const struct panel *panel, double px, double py);
...
static bool desktop_launch_desktop_file(const char *path);
static void desktop_apply_menu_action(struct app *app, int action);
static void desktop_open_context_menu(struct app *app, const struct panel *panel,
int item_idx, double pointer_x, double pointer_y);
...
style->window_transparency = 12;
style->desktop_effects = true;

shell_style_apply_theme(style);
related_surface_alpha_multiplier = shell_style_related_alpha_multiplier(style);
}
...
static double
shell_style_related_alpha_multiplier(const struct shell_style *style)
{
if (!style) {
return 1.0;
}
...
double factor = (double)style->window_transparency / 40.0;
double multiplier = 1.0 - factor * 0.16;
if (multiplier < 0.84) {
multiplier = 0.84;
}
if (multiplier > 1.0) {
...
static double
open_animation_progress(const struct app *app, uint64_t started_ms)
{
if (!app || !app->style.desktop_effects || started_ms == 0) {
return 1.0;
}
...
double t = (double)(now - started_ms) / 180.0;
return ease_out_cubic(t);
}

static double
launcher_open_animation_progress(const struct app *app)
{
if (!app || !app->launcher_open) {
return 1.0;
}
return open_animation_progress(app, app->launcher_open_started_ms);
}

static double
top_popup_open_animation_progress(const struct app *app)
{
if (!app || !app->quick_open) {
return 1.0;
}
return open_animation_progress(app, app->top_popup_open_started_ms);
}

static bool
runtime_animation_active(const struct app *app)
{
if (!app || !app->style.desktop_effects) {
return false;
}

if (app->launcher_open && launcher_open_animation_progress(app) < 0.999) {
return true;
}
if (app->quick_open && top_popup_open_animation_progress(app) < 0.999) {
return true;
}
return false;
}
...
if (!strcmp(name, "--window-transparency")
&& parse_u32_value(value, 0, 40, &metric)) {
style->window_transparency = metric;
return;
}
if (!strcmp(name, "--desktop-effects")) {
...
static bool
desktop_positions_path(char *out, size_t out_size)
{
if (!out || out_size == 0) {
return false;
}

karton_get_config_path(out, out_size, "desktop-positions");
return out[0] != '\0';
}

static const char *
desktop_output_name_from_slot(const struct app *app, int slot)
{
if (!app || slot < 0 || slot >= (int)app->output_count) {
return NULL;
}

return app->output_states[slot].name[0] ? app->output_states[slot].name : NULL;
}

static int
desktop_output_slot_from_name(const struct app *app, const char *name)
{
if (!app || !name || !*name) {
return -1;
}
...
if (!strcmp(app->output_states[i].name, name)) {
return (int)i;
}
}
...
static void
desktop_item_assign_output(struct app *app, struct desktop_item *item,
int slot, const char *output_name)
{
if (!item) {
return;
...
int resolved_slot = slot < 0 ? 0 : slot;
const char *resolved_name = output_name;
if (resolved_name && *resolved_name) {
int matched_slot = desktop_output_slot_from_name(app, resolved_name);
if (matched_slot >= 0) {
resolved_slot = matched_slot;
}
} else {
resolved_name = desktop_output_name_from_slot(app, resolved_slot);
}

item->output_slot = resolved_slot;
snprintf(item->output_name, sizeof(item->output_name), "%s",
resolved_name ? resolved_name : "");
}

static void
desktop_item_set_themed_icon_name(struct desktop_item *item, GIcon *icon)
{
if (!item || !icon || !G_IS_THEMED_ICON(icon)) {
return;
}
...
snprintf(item->icon_name, sizeof(item->icon_name), "%s", names[i]);
return;
}
}

static const char *
desktop_icon_fallback_name(const char *path, const char *content_type, bool executable)
{
...
if (!g_ascii_strncasecmp(content_type, "video/", 6)) {
return "video-x-generic";
}
if (!g_ascii_strcasecmp(content_type, "application/pdf")) {
return "application-pdf";
}
if (contains_nocase(content_type, "zip")
|| contains_nocase(content_type, "tar")
|| contains_nocase(content_type, "rar")
|| contains_nocase(content_type, "7z")
|| contains_nocase(content_type, "gzip")
|| contains_nocase(content_type, "bzip")
|| contains_nocase(content_type, "xz")
|| contains_nocase(content_type, "compressed")
|| contains_nocase(content_type, "archive")) {
return "package-x-generic";
}
if (contains_nocase(content_type, "shellscript")
|| contains_nocase(content_type, "x-script")
|| contains_nocase(content_type, "python")
|| contains_nocase(content_type, "perl")
|| contains_nocase(content_type, "ruby")
|| contains_nocase(content_type, "javascript")) {
return "text-x-script";
}
if (contains_nocase(content_type, "executable")
|| contains_nocase(content_type, "binary")) {
return "application-x-executable";
}
if (!g_ascii_strncasecmp(content_type, "text/", 5)) {
return executable ? "text-x-script" : "text-x-generic";
}
}

if (path && *path) {
if (str_ends_with(path, ".zip") || str_ends_with(path, ".tar")
|| str_ends_with(path, ".tar.gz") || str_ends_with(path, ".tgz")
|| str_ends_with(path, ".tar.xz") || str_ends_with(path, ".txz")
|| str_ends_with(path, ".7z") || str_ends_with(path, ".rar")
|| str_ends_with(path, ".gz") || str_ends_with(path, ".bz2")
|| str_ends_with(path, ".xz")) {
return "package-x-generic";
}
if (str_ends_with(path, ".sh") || str_ends_with(path, ".py")
|| str_ends_with(path, ".pl") || str_ends_with(path, ".rb")
|| str_ends_with(path, ".js") || str_ends_with(path, ".lua")) {
return "text-x-script";
}
}
...
static void
desktop_item_resolve_icon(struct desktop_item *item)
{
if (!item) {
return;
}
struct stat st = { 0 };
bool executable = stat(item->path, &st) == 0
&& S_ISREG(st.st_mode)
&& (st.st_mode & (S_IXUSR | S_IXGRP | S_IXOTH));

item->icon_name[0] = '\0';
if (item->is_dir) {
snprintf(item->icon_name, sizeof(item->icon_name), "%s", "folder");
return;
}

if (str_ends_with(item->path, ".desktop")) {
GKeyFile *key_file = g_key_file_new();
if (g_key_file_load_from_file(key_file, item->path, G_KEY_FILE_NONE, NULL)) {
char *icon_name = g_key_file_get_locale_string(key_file,
"Desktop Entry", "Icon", NULL, NULL);
if (icon_name && *icon_name) {
snprintf(item->icon_name, sizeof(item->icon_name), "%s", icon_name);
}
g_free(icon_name);
}
...
snprintf(item->icon_name, sizeof(item->icon_name), "%s", "application-x-executable");
return;
}

gboolean uncertain = FALSE;
char *content_type = g_content_type_guess(item->path, NULL, 0, &uncertain);
if (content_type && *content_type) {
GIcon *icon = g_content_type_get_icon(content_type);
desktop_item_set_themed_icon_name(item, icon);
g_clear_object(&icon);
}

if (!item->icon_name[0]) {
snprintf(item->icon_name, sizeof(item->icon_name), "%s",
desktop_icon_fallback_name(item->path, content_type, executable));
}
...
static bool
desktop_launch_in_terminal(const char *command, const char *cwd, GError **error)
{
if (!command || !*command) {
return false;
}
...
const char *home = getenv("HOME");
char local_terminal[PATH_MAX] = { 0 };
if (home && *home) {
snprintf(local_terminal, sizeof(local_terminal), "%s/.local-karton/bin/karton-terminal", home);
}

char *argv_karton[6] = { NULL, NULL, NULL, NULL, NULL, NULL };
if (local_terminal[0] && access(local_terminal, X_OK) == 0) {
argv_karton[0] = local_terminal;
} else if (program_in_path("karton-terminal")) {
argv_karton[0] = "karton-terminal";
}
if (argv_karton[0]) {
argv_karton[1] = "-e";
argv_karton[2] = (char *)command;
if (cwd && *cwd) {
argv_karton[3] = "--working-directory";
argv_karton[4] = (char *)cwd;
}
if (g_spawn_async(cwd && *cwd ? cwd : NULL, argv_karton, NULL,
...
char *argv_foot[] = { "foot", "sh", "-lc", (char *)command, NULL };
if (program_in_path("foot")
&& g_spawn_async(cwd && *cwd ? cwd : NULL, argv_foot, NULL,
G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, error)) {
return true;
}
g_clear_error(error);

char *argv_alacritty[] = { "alacritty", "-e", "sh", "-lc", (char *)command, NULL };
if (program_in_path("alacritty")
&& g_spawn_async(cwd && *cwd ? cwd : NULL, argv_alacritty, NULL,
G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, error)) {
return true;
}
g_clear_error(error);

char *argv_xterm[] = { "xterm", "-e", "sh", "-lc", (char *)command, NULL };
if (program_in_path("xterm")
&& g_spawn_async(cwd && *cwd ? cwd : NULL, argv_xterm, NULL,
G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, error)) {
return true;
}
g_clear_error(error);

char *argv_konsole[] = { "konsole", "-e", "sh", "-lc", (char *)command, NULL };
if (program_in_path("konsole")
&& g_spawn_async(cwd && *cwd ? cwd : NULL, argv_konsole, NULL,
G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, error)) {
return true;
}
g_clear_error(error);

char *argv_gnome[] = { "gnome-terminal", "--", "sh", "-lc", (char *)command, NULL };
if (program_in_path("gnome-terminal")
&& g_spawn_async(cwd && *cwd ? cwd : NULL, argv_gnome, NULL,
G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, error)) {
return true;
}
g_clear_error(error);
...
static void
desktop_positions_load(struct app *app)
{
if (!app) {
return;
}

app->desktop_position_count = 0;

char path[PATH_MAX] = { 0 };
if (!desktop_positions_path(path, sizeof(path))) {
return;
}
...
while (fgets(line, sizeof(line), f)) {
if (app->desktop_position_count >= MAX_DESKTOP_ITEMS) {
break;
}
...
char *x_text = trim_in_place(tab1);
char *y_text = tab2;
char *tab3 = strchr(y_text, '\t');
char *slot_text = NULL;
char *output_name = NULL;
if (tab3) {
*tab3++ = '\0';
slot_text = trim_in_place(tab3);
char *tab4 = strchr(slot_text, '\t');
if (tab4) {
*tab4++ = '\0';
output_name = trim_in_place(tab4);
}
}

y_text = trim_in_place(y_text);
char *end_x = NULL;
char *end_y = NULL;
errno = 0;
double x = strtod(x_text, &end_x);
double y = strtod(y_text, &end_y);
if (errno != 0 || !end_x || *trim_in_place(end_x) != '\0'
|| !end_y || *trim_in_place(end_y) != '\0') {
continue;
}

struct desktop_position *pos = &app->desktop_positions[app->desktop_position_count];
snprintf(pos->path, sizeof(pos->path), "%s", trimmed);
pos->output_slot = 0;
pos->output_name[0] = '\0';
if (slot_text && *slot_text) {
char *end_slot = NULL;
errno = 0;
long parsed_slot = strtol(slot_text, &end_slot, 10);
if (errno == 0 && end_slot && *trim_in_place(end_slot) == '\0' && parsed_slot >= 0) {
pos->output_slot = (int)parsed_slot;
}
}
if (output_name && *output_name) {
snprintf(pos->output_name, sizeof(pos->output_name), "%s", output_name);
} else {
app->desktop_positions_need_migration = true;
}
pos->x = x;
pos->y = y;
app->desktop_position_count++;
}
...
static void
desktop_positions_save(const struct app *app)
{
if (!app) {
return;
}
...
for (size_t i = 0; i < app->desktop_item_count; i++) {
const struct desktop_item *item = &app->desktop_items[i];
if (!item->has_custom_pos || !item->path[0]) {
continue;
}
if (item->output_name[0]) {
fprintf(f, "%s\t%.1f\t%.1f\t%d\t%s\n", item->path, item->custom_x, item->custom_y,
item->output_slot < 0 ? 0 : item->output_slot, item->output_name);
} else {
fprintf(f, "%s\t%.1f\t%.1f\t%d\n", item->path, item->custom_x, item->custom_y,
item->output_slot < 0 ? 0 : item->output_slot);
}
}
...
static void
desktop_apply_saved_positions(struct app *app)
{
if (!app) {
return;
}

desktop_positions_load(app);

for (size_t i = 0; i < app->desktop_item_count; i++) {
struct desktop_item *item = &app->desktop_items[i];
desktop_item_assign_output(app, item, 0, NULL);
item->has_custom_pos = false;
item->custom_x = 0.0;
item->custom_y = 0.0;

for (size_t j = 0; j < app->desktop_position_count; j++) {
const struct desktop_position *pos = &app->desktop_positions[j];
if (strcmp(pos->path, item->path) != 0) {
continue;
}
item->has_custom_pos = true;
desktop_item_assign_output(app, item,
pos->output_slot < 0 ? 0 : pos->output_slot,
pos->output_name);
item->custom_x = pos->x;
item->custom_y = pos->y;
break;
...
if (app->desktop_positions_need_migration && app->output_count > 0) {
desktop_positions_save(app);
app->desktop_positions_need_migration = false;
}
}
...
desktop_item_resolve_icon(item);
...
desktop_apply_saved_positions(app);
...
static bool
desktop_refresh_if_needed(struct app *app)
{
if (!app) {
return false;
}
...
uint64_t now = monotonic_msec();
if (app->desktop_last_scan_ms != 0 && now - app->desktop_last_scan_ms < 750) {
return false;
}
app->desktop_last_scan_ms = now;

char dir[PATH_MAX] = { 0 };
if (!desktop_resolve_dir(dir, sizeof(dir))) {
if (app->desktop_item_count != 0 || app->desktop_dir[0]) {
app->desktop_item_count = 0;
app->desktop_dir[0] = '\0';
app->desktop_dir_mtime_valid = false;
return true;
}
return false;
}
...
if (app->desktop_item_count != 0 || strcmp(app->desktop_dir, dir) != 0) {
app->desktop_item_count = 0;
snprintf(app->desktop_dir, sizeof(app->desktop_dir), "%s", dir);
app->desktop_dir_mtime_valid = false;
return true;
}
return false;
...
bool changed = strcmp(app->desktop_dir, dir) != 0
|| !app->desktop_dir_mtime_valid
|| app->desktop_dir_mtime.tv_sec != st.st_mtim.tv_sec
|| app->desktop_dir_mtime.tv_nsec != st.st_mtim.tv_nsec;
if (!changed) {
return false;
}

snprintf(app->desktop_dir, sizeof(app->desktop_dir), "%s", dir);
app->desktop_dir_mtime = st.st_mtim;
app->desktop_dir_mtime_valid = true;
desktop_items_load(app);
return true;
}

static void
desktop_clamp_item_position(const struct panel *panel, const struct app *app,
double *x, double *y, double w, double h)
{
if (!panel || !app || !x || !y) {
return;
...
double min_x = (panel->reserve_shell_margins ? app->style.side_width : 0) + 16.0;
double min_y = (panel->reserve_shell_margins ? app->style.top_height : 0) + 16.0;
double max_x = panel->width - w - 16.0;
double max_y = panel->height - h - 16.0;
...
const double start_x = (panel->reserve_shell_margins ? app->style.side_width : 0) + 16.0;
const double start_y = (panel->reserve_shell_margins ? app->style.top_height : 0) + 16.0;
int columns = (int)((panel->width - start_x - 16.0) / cell_w);
if (columns < 1) {
columns = 1;
}

const struct desktop_item *item = &app->desktop_items[idx];
int panel_slot = desktop_panel_slot(app, panel);
int item_slot = item->output_slot < 0 ? 0 : item->output_slot;
if (item_slot != panel_slot) {
int named_slot = desktop_output_slot_from_name(app, item->output_name);
if (named_slot >= 0) {
item_slot = named_slot;
}
}
if (item_slot != panel_slot) {
return false;
...
if (item->has_custom_pos) {
double item_x = item->custom_x;
double item_y = item->custom_y;
desktop_clamp_item_position(panel, app, &item_x, &item_y,
cell_w - 10.0, cell_h - 8.0);
*x = item_x;
*y = item_y;
*w = cell_w - 10.0;
*h = cell_h - 8.0;
...
int visible_idx = 0;
for (int i = 0; i < idx; i++) {
const struct desktop_item *prev = &app->desktop_items[i];
int prev_slot = prev->output_slot < 0 ? 0 : prev->output_slot;
if (prev_slot == panel_slot && !prev->has_custom_pos) {
visible_idx++;
}
}

int row = visible_idx / columns;
int col = visible_idx % columns;
...
if (desktop_launch_desktop_file(path)) {
return;
}
...
if (item->is_dir) {
const char *home = getenv("HOME");
char local_bin[PATH_MAX] = { 0 };
if (home && *home) {
snprintf(local_bin, sizeof(local_bin), "%s/.local-karton/bin/karton-files", home);
if (access(local_bin, X_OK) == 0) {
execl(local_bin, "karton-files", path, (char *)NULL);
}
}
execlp("karton-files", "karton-files", path, (char *)NULL);
}
...
static void
update_desktop_input_region(struct panel *panel)
{
if (!panel || panel->type != PANEL_DESKTOP || !panel->app
|| !panel->app->compositor || !panel->surface) {
return;
}

struct wl_region *region = wl_compositor_create_region(panel->app->compositor);
if (!region) {
return;
}

for (int i = 0; i < (int)panel->app->desktop_item_count; i++) {
double x, y, w, h;
if (!desktop_item_rect(panel, panel->app, i, &x, &y, &w, &h)) {
continue;
}
...
if (desktop_menu_rect(panel->app, panel, &menu_x, &menu_y, &menu_w, &menu_h)) {
wl_region_add(region, (int)menu_x, (int)menu_y, (int)menu_w, (int)menu_h);
}

wl_surface_set_input_region(panel->surface, region);
...
if (panel->app->style.desktop_effects && panel->app->launcher_open) {
double progress = launcher_open_animation_progress(panel->app);
if (progress < 0.999) {
double min_w = *w * 0.38;
if (min_w < 220.0) {
min_w = 220.0;
...
double animated_w = min_w + (*w - min_w) * progress;
*x += (*w - animated_w);
*w = animated_w;
}
}
...
static bool
desktop_menu_rect(const struct app *app, const struct panel *panel,
double *x, double *y, double *w, double *h)
{
if (!app || !panel || !app->desktop_menu_open
|| desktop_panel_slot(app, panel) != app->desktop_menu_panel_slot) {
return false;
}

if (x) {
*x = app->desktop_menu_x;
}
if (y) {
*y = app->desktop_menu_y;
}
if (w) {
*w = 212.0;
}
if (h) {
*h = DESKTOP_MENU_ITEM_COUNT * 28.0 + 8.0;
}
return true;
}

static int
desktop_menu_item_hit(const struct app *app, const struct panel *panel, double px, double py)
{
double x, y, w, h;
if (!desktop_menu_rect(app, panel, &x, &y, &w, &h)) {
return -1;
}
...
int idx = (int)((py - y - 4.0) / 28.0);
if (idx < 0 || idx >= DESKTOP_MENU_ITEM_COUNT) {
return -1;
}
return idx;
}
...
static char *
desktop_exec_without_field_codes(const char *exec)
{
if (!exec || !*exec) {
return NULL;
}
...
static bool
desktop_launch_desktop_file(const char *path)
{
if (!path || !*path || !str_ends_with(path, ".desktop")) {
return false;
}
...
show_notification_message(_("Desktop"),
error && error->message ? error->message : _("Could not open desktop shortcut"));
g_clear_error(&error);
g_key_file_unref(key_file);
return true;
}

char *type = g_key_file_get_string(key_file, "Desktop Entry", "Type", NULL);
...
char *cwd = g_key_file_get_string(key_file, "Desktop Entry", "Path", NULL);
bool needs_terminal = g_key_file_get_boolean(key_file, "Desktop Entry", "Terminal", NULL);
char *sanitized = desktop_exec_without_field_codes(exec);
g_free(exec);
if (!sanitized || !*sanitized) {
g_free(sanitized);
g_free(cwd);
g_key_file_unref(key_file);
show_notification_message(_("Desktop"), _("Shortcut has no executable command"));
return true;
}

gchar **argv = NULL;
if (!needs_terminal && !g_shell_parse_argv(sanitized, NULL, &argv, &error)) {
show_notification_message(_("Desktop"),
error && error->message ? error->message : _("Could not parse shortcut command"));
g_clear_error(&error);
g_free(sanitized);
g_free(cwd);
g_key_file_unref(key_file);
...
if (needs_terminal) {
spawned = desktop_launch_in_terminal(sanitized, cwd, &error);
} else {
...
if (!spawned) {
show_notification_message(_("Desktop"),
error && error->message ? error->message : _("Could not launch shortcut"));
g_clear_error(&error);
}
...
static gboolean
desktop_delete_recursive_file(GFile *file, GError **error)
{
if (!file) {
return TRUE;
}
...
gboolean ok = desktop_delete_recursive_file(child, error);
...
static char *
desktop_prompt_rename(const char *current_name)
{
...
tool = "zenity";
} else if (program_in_path("qarma")) {
tool = "qarma";
} else if (program_in_path("yad")) {
tool = "yad";
} else {
show_notification_message(_("Desktop"), _("Rename requires zenity, qarma or yad"));
return NULL;
}

gchar *quoted_title = g_shell_quote(_("Rename item"));
gchar *quoted_text = g_shell_quote(_("Enter a new name:"));
gchar *quoted_name = g_shell_quote(current_name);
char command[1024] = { 0 };
snprintf(command, sizeof(command),
"%s --entry --title=%s --text=%s --entry-text=%s",
tool, quoted_title, quoted_text, quoted_name);
g_free(quoted_title);
g_free(quoted_text);
g_free(quoted_name);
...
char stderr_text[256] = { 0 };
if (!run_command_line_sync(command, stdout_text, sizeof(stdout_text), stderr_text, sizeof(stderr_text))) {
return NULL;
}

char *trimmed = trim_in_place(stdout_text);
if (!trimmed[0]) {
return NULL;
}
...
static void
desktop_refresh_after_item_change(struct app *app, const char *new_path, bool preserve_position,
int old_output_slot, const char *old_output_name, double old_x, double old_y)
{
if (!app) {
return;
}
...
desktop_items_load(app);
if (preserve_position && new_path && *new_path) {
for (size_t i = 0; i < app->desktop_item_count; i++) {
struct desktop_item *item = &app->desktop_items[i];
if (strcmp(item->path, new_path) != 0) {
continue;
}
desktop_item_assign_output(app, item, old_output_slot, old_output_name);
item->has_custom_pos = true;
item->custom_x = old_x;
item->custom_y = old_y;
break;
...
desktop_positions_save(app);
app->desktop_hover_item = -1;
app->desktop_menu_open = false;
app->desktop_menu_target = -1;
app->desktop_menu_hover = -1;
draw_desktop_panels(app);
}

static void
desktop_apply_menu_action(struct app *app, int action)
{
if (!app || app->desktop_menu_target < 0
|| app->desktop_menu_target >= (int)app->desktop_item_count) {
return;
}

struct desktop_item *item = &app->desktop_items[app->desktop_menu_target];
int old_output_slot = item->output_slot;
char old_output_name[64] = { 0 };
snprintf(old_output_name, sizeof(old_output_name), "%s", item->output_name);
double old_x = item->custom_x;
double old_y = item->custom_y;

switch (action) {
case 0:
desktop_open_item(app, app->desktop_menu_target);
app->desktop_menu_open = false;
app->desktop_menu_hover = -1;
draw_desktop_panels(app);
return;
case 1: {
char *new_name = desktop_prompt_rename(item->name);
if (!new_name) {
return;
}
if (!strcmp(new_name, item->name)) {
g_free(new_name);
app->desktop_menu_open = false;
app->desktop_menu_hover = -1;
draw_desktop_panels(app);
...
GFile *src = g_file_new_for_path(item->path);
GFile *parent = g_file_get_parent(src);
GFile *dest = parent ? g_file_get_child(parent, new_name) : NULL;
GError *error = NULL;
if (!src || !dest || !g_file_move(src, dest, G_FILE_COPY_NONE, NULL, NULL, NULL, &error)) {
show_notification_message(_("Desktop"),
error && error->message ? error->message : _("Rename failed"));
g_clear_error(&error);
} else {
char *new_path = g_file_get_path(dest);
    desktop_refresh_after_item_change(app, new_path, true,
old_output_slot, old_output_name, old_x, old_y);
g_free(new_path);
}
g_clear_object(&dest);
...
show_notification_message(_("Desktop"),
error && error->message ? error->message : _("Could not move item to trash"));
g_clear_error(&error);
} else {
            desktop_refresh_after_item_change(app, NULL, false, 0, NULL, 0.0, 0.0);
}
g_object_unref(file);
return;
}
...
GFile *file = g_file_new_for_path(item->path);
GError *error = NULL;
if (!desktop_delete_recursive_file(file, &error)) {
show_notification_message(_("Desktop"),
error && error->message ? error->message : _("Could not delete item"));
g_clear_error(&error);
} else {
            desktop_refresh_after_item_change(app, NULL, false, 0, NULL, 0.0, 0.0);
}
...
static void
desktop_open_context_menu(struct app *app, const struct panel *panel,
int item_idx, double pointer_x, double pointer_y)
{
if (!app || !panel || item_idx < 0 || item_idx >= (int)app->desktop_item_count) {
return;
}

app->desktop_menu_open = true;
app->desktop_menu_target = item_idx;
app->desktop_menu_hover = -1;
app->desktop_menu_panel_slot = desktop_panel_slot(app, panel);
app->desktop_menu_x = pointer_x + 8.0;
app->desktop_menu_y = pointer_y + 6.0;

double menu_w = 212.0;
double menu_h = DESKTOP_MENU_ITEM_COUNT * 28.0 + 8.0;
double max_x = panel->width - menu_w - 12.0;
double max_y = panel->height - menu_h - 12.0;
if (app->desktop_menu_x > max_x) {
app->desktop_menu_x = max_x;
}
if (app->desktop_menu_y > max_y) {
app->desktop_menu_y = max_y;
}
if (app->desktop_menu_x < 12.0) {
app->desktop_menu_x = 12.0;
}
if (app->desktop_menu_y < 12.0) {
app->desktop_menu_y = 12.0;
}
...
{
fprintf(stderr,
_("Usage: %s [--top-only|--side-only|--desktop-only]\n"
"  --top-only   Run only the top panel\n"
"  --side-only  Run only the side dock\n"
"  --desktop-only  Run only the desktop surface\n"),
argv0);
}
...
if (!strcmp(argv[i], "--desktop-only")) {
if (*mode != RUN_BOTH) {
return false;
}
*mode = RUN_DESKTOP_ONLY;
continue;
}
...
double effective_alpha = alpha * related_surface_alpha_multiplier;
...
cairo_set_source_rgba(cairo, r, g, b, effective_alpha);
}
...
static void
draw_settings_icon(cairo_t *cairo, double cx, double cy, double size,
uint32_t color, double alpha)
{
const double pi = 3.14159265358979323846;
cairo_save(cairo);
...
for (int i = 0; i < 8; i++) {
double a = ((double)i / 8.0) * 2.0 * pi;
double x1 = cx + cos(a) * (outer - size * 0.06);
double y1 = cy + sin(a) * (outer - size * 0.06);
double x2 = cx + cos(a) * outer;
double y2 = cy + sin(a) * outer;
cairo_move_to(cairo, x1, y1);
cairo_line_to(cairo, x2, y2);
}
cairo_stroke(cairo);

cairo_arc(cairo, cx, cy, outer - size * 0.08, 0, 2.0 * pi);
cairo_stroke(cairo);

cairo_arc(cairo, cx, cy, inner, 0, 2.0 * pi);
cairo_fill(cairo);
...
const double centers[] = { x + 14.0, x + 42.0, x + 70.0, x + 98.0 };
...
draw_karton_symbol(cairo, 3, centers[0], cy, 18.0, colors[0], dark ? 1.0 : 0.96);
if (has_unread_notification) {
    set_source_hex_a(cairo, dark ? 0x172031 : 0xffffff, dark ? 0.64 : 0.92);
    cairo_arc(cairo, centers[0] + 9.0, cy - 10.0, 5.2, 0, 2.0 * 3.14159265358979323846);
    cairo_fill(cairo);
set_source_hex_a(cairo, 0xf05d7b, 0.96);
cairo_arc(cairo, centers[0] + 9.0, cy - 10.0, 3.8, 0, 2.0 * 3.14159265358979323846);
cairo_fill(cairo);
}
draw_wifi_icon(cairo, centers[1], cy, 18.0, colors[1], dark ? 1.0 : 0.96);
draw_speaker_icon(cairo, centers[2], cy, 18.0, colors[2], dark ? 1.0 : 0.96);
draw_battery_icon(cairo, centers[3] - 6.0, cy - 7.0, 22.0, 14.0, 0.72, colors[3], dark ? 1.0 : 0.96);
}
...
static void
draw_desktop_panels(struct app *app)
{
if (!app) {
return;
}
...
for (size_t i = 0; i < app->desktop_extra_count; i++) {
panel_draw(&app->desktop_extra[i]);
}
}

static struct panel *
desktop_panel_from_surface(struct app *app, struct wl_surface *surface)
{
if (!app || !surface) {
return NULL;
}
...
for (size_t i = 0; i < app->desktop_extra_count; i++) {
if (surface == app->desktop_extra[i].surface) {
return &app->desktop_extra[i];
}
}
...
static int
desktop_panel_slot(const struct app *app, const struct panel *panel)
{
if (!app || !panel || !panel->bound_output) {
return 0;
}

for (size_t i = 0; i < app->output_count; i++) {
if (app->outputs[i] == panel->bound_output) {
return (int)i;
}
}
...
top_quick_panel_content_height(const struct panel *panel, double avail_h)
{
    ...
    if (app->top_popup_mode == TOP_POPUP_QUICK) {
        desired_h = 582.0;
        desired_h = clamp_double(desired_h, 582.0, avail_h);
        return desired_h;
    }
...
static void
apply_top_popup_open_animation(const struct panel *panel, double *y, double *h)
{
if (!panel || !panel->app || !y || !h) {
return;
}
if (!panel->app->quick_open || !panel->app->style.desktop_effects) {
return;
}

double progress = top_popup_open_animation_progress(panel->app);
double slide = (1.0 - progress) * 16.0;
*y += slide;

double max_bottom = panel->height - 8.0;
if (*y + *h > max_bottom) {
*h = max_bottom - *y;
}
if (*h < 120.0) {
...
apply_top_popup_open_animation(panel, y, h);
return true;
}
...
*w = 188.0;
*h = clock_timezone_candidate_count * 24.0 + 34.0;
if (*h > ch - 80.0) {
*h = ch - 80.0;
}
if (*h < 120.0) {
...
if (row_y + i * 24.0 + 22.0 > y + 30.0 && row_y + i * 24.0 < y + h &&
    point_in_rect(px, py, x + 8.0, row_y + i * 24.0, w - 16.0, 22.0)) {
return (int)i;
}
}
...
static int
quick_footer_button_hit(const struct panel *panel, double px, double py)
{
double qx, qy, qw, qh;
if (!top_quick_panel_rect(panel, &qx, &qy, &qw, &qh)) {
return -1;
...
if (point_in_rect(px, py, qx + 16.0, button_y, button_w, 34.0)) {
return 0;
}
if (point_in_rect(px, py, qx + 26.0 + button_w, button_y, button_w, 34.0)) {
return 1;
}
return -1;
}
...
if (point_in_rect(px, py, sx + 0.0, sy, 28.0, 24.0)) {
return 0;
}
if (point_in_rect(px, py, sx + 28.0, sy, 28.0, 24.0)) {
return 1;
}
if (point_in_rect(px, py, sx + 56.0, sy, 28.0, 24.0)) {
return 2;
}
if (point_in_rect(px, py, sx + 84.0, sy, 28.0, 24.0)) {
return 3;
}
return -1;
}
...
static void
open_authors_info(void)
{
spawn_command("sh -lc 'if [ -f \"$HOME/KartONDE/AUTHORS.md\" ]; then xdg-open \"$HOME/KartONDE/AUTHORS.md\"; elif [ -f \"$HOME/.local-karton/share/doc/KartONDE/AUTHORS.md\" ]; then xdg-open \"$HOME/.local-karton/share/doc/KartONDE/AUTHORS.md\"; elif [ -f \"/usr/share/doc/KartONDE/AUTHORS.md\" ]; then xdg-open \"/usr/share/doc/KartONDE/AUTHORS.md\"; else command -v notify-send >/dev/null 2>&1 && notify-send \"Karton\" \"Brak pliku AUTHORS.md\" || true; fi'");
}
...
draw_top_status_icons(cairo, panel->app, bx - 118.0, by + 5.0, style->top_text, dark);
...
set_source_hex_a(cairo, style->quick_panel_bg, panel_alpha);
...
strftime(month_label, sizeof(month_label), _("%B %Y"), &month_tm);
}
...
set_source_hex_a(cairo, style->side_launcher_bg, 0.18);
double quick_btn_y = qy + qh - 50.0;
double quick_btn_w = (qw - 42.0) / 2.0;
rounded_rect(cairo, qx + 16.0, quick_btn_y, quick_btn_w, 34.0, 12.0);
cairo_fill(cairo);
bool have_settings_icon = draw_named_icon(cairo, panel->app, "settings", qx + 38.0, quick_btn_y + 17.0, 15.0);
if (!have_settings_icon) {
draw_settings_icon(cairo, qx + 38.0, quick_btn_y + 17.0, 14.0, style->quick_title_text, 0.94);
}
draw_pango_text(cairo, "Noto Sans", PANGO_WEIGHT_SEMIBOLD,
10.0, style->quick_title_text, 0.94, qx + 52.0, quick_btn_y + 10.0,
(int)quick_btn_w - 36, PANGO_ALIGN_CENTER, _("Open settings"));

set_source_hex_a(cairo, dark ? 0x253555 : 0xf2f6fc, 0.94);
rounded_rect(cairo, qx + 26.0 + quick_btn_w, quick_btn_y, quick_btn_w, 34.0, 12.0);
cairo_fill(cairo);
bool have_info_icon = draw_named_icon(cairo, panel->app, "info", qx + 26.0 + quick_btn_w + 22.0, quick_btn_y + 17.0, 14.0);
if (!have_info_icon) {
draw_pango_text(cairo, "Noto Sans", PANGO_WEIGHT_BOLD,
11.0, style->quick_title_text, 0.92, qx + 26.0 + quick_btn_w + 16.0, quick_btn_y + 8.0,
14, PANGO_ALIGN_CENTER, "i");
}
draw_pango_text(cairo, "Noto Sans", PANGO_WEIGHT_SEMIBOLD,
10.0, style->quick_title_text, 0.94, qx + 26.0 + quick_btn_w + 34.0, quick_btn_y + 10.0,
(int)quick_btn_w - 40, PANGO_ALIGN_CENTER, "O autorach");
...
bool dark = theme_mode_is_dark(style->theme_mode);
double side_alpha = shell_style_surface_alpha(style, dark ? 0.90 : 0.97);
double popup_alpha = shell_style_surface_alpha(style, dark ? 0.97 : 0.985);
...
set_source_hex_a(cairo, style->side_bg, side_alpha);
...
set_source_hex_a(cairo, style->side_popup_bg, popup_alpha);
...
bool have_icon = false;
if (app->desktop_items[i].icon_name[0]) {
have_icon = draw_named_icon(cairo, app, app->desktop_items[i].icon_name,
x + w * 0.5, y + 30.0, 42.0);
}
if (!have_icon) {
...
if (app->desktop_menu_open && desktop_panel_slot(app, panel) == app->desktop_menu_panel_slot) {
const char *menu_items[DESKTOP_MENU_ITEM_COUNT] = {
_("Open"),
_("Rename"),
_("Move to trash"),
_("Delete permanently"),
};
const double menu_w = 212.0;
const double row_h = 28.0;

set_source_hex_a(cairo, 0xf6f9ff, 0.98);
rounded_rect(cairo, app->desktop_menu_x, app->desktop_menu_y,
menu_w, DESKTOP_MENU_ITEM_COUNT * row_h + 8.0, 9.0);
cairo_fill(cairo);

for (int i = 0; i < DESKTOP_MENU_ITEM_COUNT; i++) {
double item_y = app->desktop_menu_y + 4.0 + i * row_h;
if (i == app->desktop_menu_hover) {
set_source_hex_a(cairo, 0x2c5aa0, 0.20);
rounded_rect(cairo, app->desktop_menu_x + 4.0, item_y,
menu_w - 8.0, row_h - 1.0, 6.0);
cairo_fill(cairo);
}

draw_pango_text(cairo, "Noto Sans", PANGO_WEIGHT_NORMAL,
10.0, 0x20324d, 0.96, app->desktop_menu_x + 10.0, item_y + 8.0,
(int)menu_w - 20, PANGO_ALIGN_LEFT, menu_items[i]);
}
}
}
...
update_desktop_input_region(panel);
}
...
app->top_popup_open_started_ms = 0;
...
app->quick_open = true;
app->top_popup_open_started_ms = monotonic_msec();
...
int footer_button = quick_footer_button_hit(&app->top, app->pointer_x, app->pointer_y);
if (footer_button == 0) {
...
if (pick >= 0
&& pick < (int)clock_timezone_candidate_count) {
...
app->launcher_open_started_ms = 0;
app->popup_group = -1;
app->popup_hover_item = -1;
app->popup_selected_item = -1;
app->launcher_hover_item = -1;
...
struct app *app = state ? state->app : NULL;
(void)output;
(void)refresh;
(void)width;
...
{
struct app *app = data;
struct panel *desktop_panel = NULL;
(void)wl_pointer;
...
desktop_panel = desktop_panel_from_surface(app, surface);
if (desktop_panel) {
desktop_items_load(app);
app->desktop_hover_item = desktop_item_hit(desktop_panel, app, app->pointer_x, app->pointer_y);
panel_draw(desktop_panel);
}
...
{
struct app *app = data;
struct panel *desktop_panel = desktop_panel_from_surface(app, app->pointer_surface);
(void)wl_pointer;
(void)time;
app->pointer_x = wl_fixed_to_double(sx);
app->pointer_y = wl_fixed_to_double(sy);
...
int old_menu_hover = app->desktop_menu_hover;
if (app->desktop_drag_active && app->desktop_drag_item >= 0
&& app->desktop_drag_item < (int)app->desktop_item_count) {
double x, y, w, h;
if (desktop_item_rect(desktop_panel, app, app->desktop_drag_item, &x, &y, &w, &h)) {
struct desktop_item *item = &app->desktop_items[app->desktop_drag_item];
double new_x = app->pointer_x - app->desktop_drag_offset_x;
double new_y = app->pointer_y - app->desktop_drag_offset_y;
desktop_clamp_item_position(desktop_panel, app, &new_x, &new_y, w, h);
desktop_item_assign_output(app, item, desktop_panel_slot(app, desktop_panel), NULL);
if (!item->has_custom_pos || item->custom_x != new_x || item->custom_y != new_y) {
item->has_custom_pos = true;
item->custom_x = new_x;
item->custom_y = new_y;
app->desktop_drag_moved = true;
draw_desktop_panels(app);
}
}
} else {
app->desktop_menu_hover = desktop_menu_item_hit(app, desktop_panel, app->pointer_x, app->pointer_y);
app->desktop_hover_item = desktop_item_hit(desktop_panel, app, app->pointer_x, app->pointer_y);
if (old_hover != app->desktop_hover_item || old_menu_hover != app->desktop_menu_hover) {
panel_draw(desktop_panel);
}
}
}
...
static void
desktop_finish_drag(struct app *app, bool save_positions)
{
if (!app || !app->desktop_drag_active) {
return;
}

app->desktop_drag_active = false;
if (save_positions && app->desktop_drag_moved) {
desktop_positions_save(app);
}
app->desktop_drag_moved = false;
app->desktop_drag_item = -1;
}
...
if (state != WL_POINTER_BUTTON_STATE_PRESSED) {
    if (state == WL_POINTER_BUTTON_STATE_RELEASED) {
        app->top_slider_drag_target = TOP_SLIDER_DRAG_NONE;
        if (button == BTN_LEFT && app->desktop_drag_active) {
            bool moved = app->desktop_drag_moved;
            int dragged_item = app->desktop_drag_item;
            desktop_finish_drag(app, true);
            if (!moved && desktop_panel && dragged_item >= 0) {
                desktop_open_item(app, dragged_item);
            }
            if (desktop_panel) {
                app->desktop_hover_item = desktop_item_hit(desktop_panel, app,
                    app->pointer_x, app->pointer_y);
                panel_draw(desktop_panel);
            }
        }
...
if (app->desktop_menu_open && !desktop_panel) {
app->desktop_menu_open = false;
app->desktop_menu_hover = -1;
app->desktop_menu_target = -1;
draw_desktop_panels(app);
...
if (desktop_panel) {
if (button == BTN_RIGHT) {
desktop_refresh_if_needed(app);
int hit = desktop_item_hit(desktop_panel, app, app->pointer_x, app->pointer_y);
if (hit >= 0) {
desktop_open_context_menu(app, desktop_panel, hit, app->pointer_x, app->pointer_y);
} else if (app->desktop_menu_open) {
app->desktop_menu_open = false;
app->desktop_menu_hover = -1;
app->desktop_menu_target = -1;
draw_desktop_panels(app);
...
if (button == BTN_LEFT) {
if (app->desktop_menu_open) {
int menu_item = desktop_menu_item_hit(app, desktop_panel, app->pointer_x, app->pointer_y);
if (menu_item >= 0) {
desktop_apply_menu_action(app, menu_item);
return;
}
...
if (hit >= 0) {
double x, y, w, h;
if (desktop_item_rect(desktop_panel, app, hit, &x, &y, &w, &h)) {
app->desktop_drag_active = true;
app->desktop_drag_moved = false;
app->desktop_drag_item = hit;
app->desktop_drag_offset_x = app->pointer_x - x;
app->desktop_drag_offset_y = app->pointer_y - y;
app->desktop_hover_item = hit;
panel_draw(desktop_panel);
}
}
}
...
if (app->pointer_surface == app->top.surface) {
    ...
    if (app->clock_picker_open) {
        app->clock_picker_scroll -= step * 3.0;
        if (app->clock_picker_scroll < 0) app->clock_picker_scroll = 0;
        double max_scroll = (clock_timezone_candidate_count * 28.0) - 200.0;
...
struct wl_output *output = wl_registry_bind(wl_registry, name,
&wl_output_interface, bind_version);
if (!output) {
return;
}

if (app->output_count < MAX_WL_OUTPUTS) {
size_t idx = app->output_count++;
app->outputs[idx] = output;
app->output_states[idx].app = app;
app->output_states[idx].output = output;
app->output_states[idx].name[0] = '\0';
wl_output_add_listener(output, &output_listener, &app->output_states[idx]);
}
...
struct wl_output *output, bool reserve_shell_margins)
{
uint32_t anchors = 0;
...
panel->type = type;
panel->app = app;
panel->bound_output = output;
panel->reserve_shell_margins = reserve_shell_margins;
...
zwlr_layer_surface_v1_set_anchor(panel->layer_surface, anchors);
zwlr_layer_surface_v1_set_margin(panel->layer_surface,
panel->reserve_shell_margins ? app->style.top_height : 0,
0,
0,
panel->reserve_shell_margins && app->side_enabled ? app->style.side_width : 0);
...
bool need_top = mode != RUN_SIDE_ONLY && mode != RUN_DESKTOP_ONLY;
bool need_side = mode != RUN_TOP_ONLY && mode != RUN_DESKTOP_ONLY;
bool need_desktop = mode == RUN_BOTH || mode == RUN_DESKTOP_ONLY;
...
app.side.app = &app;
app.desktop.app = &app;
for (size_t i = 0; i < MAX_WL_OUTPUTS - 1; i++) {
app.desktop_extra[i].app = &app;
}
load_icon_theme_name(&app);
calendar_set_today(&app);
load_launcher_entries(&app);
clock_load_available_timezones();
clock_load_timezones(&app);
shell_style_apply_theme(&app.style);
...
if (need_desktop) {
desktop_items_load(&app);
app.desktop_last_scan_ms = monotonic_msec();
}
...
bool desktop_create_failed = false;
if ((need_top && !panel_create(&app, &app.top, PANEL_TOP, app.output, true))
|| (need_side && !panel_create(&app, &app.side, PANEL_SIDE, app.output, true))) {
desktop_create_failed = true;
}

if (!desktop_create_failed && need_desktop) {
struct wl_output *primary_output = app.output_count > 0 ? app.outputs[0] : app.output;
if (!panel_create(&app, &app.desktop, PANEL_DESKTOP, primary_output,
    need_top || need_side)) {
desktop_create_failed = true;
}

for (size_t i = 1; !desktop_create_failed && i < app.output_count; i++) {
if (app.desktop_extra_count >= MAX_WL_OUTPUTS - 1) {
break;
}

struct panel *extra = &app.desktop_extra[app.desktop_extra_count];
if (!panel_create(&app, extra, PANEL_DESKTOP, app.outputs[i], false)) {
desktop_create_failed = true;
break;
}
app.desktop_extra_count++;
}
}

if (desktop_create_failed) {
fprintf(stderr, "%s\n", _("karton-shell: failed to create layer surfaces"));
if (need_desktop) {
for (size_t i = 0; i < app.desktop_extra_count; i++) {
panel_destroy(&app.desktop_extra[i]);
}
...
while (app.running) {
if (need_desktop && desktop_refresh_if_needed(&app) && app.desktop.configured) {
draw_desktop_panels(&app);
}
...
process_runtime_signals(&app);
bool anim_active = runtime_animation_active(&app);
if (need_top && (pending_rc > 0 || anim_active)) {
panel_draw(&app.top);
}
...
int poll_timeout_ms = anim_active ? 16 : 200;
...
if (runtime_animation_active(&app)) {
if (need_top) {
panel_draw(&app.top);
}
if (need_side) {
...
for (size_t i = 0; i < app.output_count; i++) {
if (app.outputs[i]) {
wl_output_destroy(app.outputs[i]);
}
}
if (app.layer_shell) {
