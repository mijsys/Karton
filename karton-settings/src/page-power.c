...
static char *apply_runtime_power(void)
{
    ...
    (void)run_command_success(
        "sh -lc 'if [ -f \"$HOME/.config/karton/environment\" ]; then set -a; . \"$HOME/.config/karton/environment\"; set +a; fi; killall karton-idle 2>/dev/null; if command -v karton-idle >/dev/null 2>&1; then karton-idle >/dev/null 2>&1 & elif [ -x \"$HOME/.local-karton/bin/karton-idle\" ]; then \"$HOME/.local-karton/bin/karton-idle\" >/dev/null 2>&1 & fi; true'" );
