...
static void apply_grid_density_for_item_count(FilesState *state, guint item_count);
static gboolean delete_recursive_file(GFile *file, GError **error);
static gboolean token_looks_like_archive(const char *token);
static gboolean extract_archive_token(FilesState *state, const char *archive_token);
static gboolean compress_token_to_archive(FilesState *state, const char *item_token);
...
static void open_directory_in_new_tab(FilesState *state, const char *token) {
    ...
    GtkWidget *new_window = karton_files_window_new(app, token);
...
static void set_clipboard_item(FilesState *state, const char *token, gboolean cut) {
    if (state->clipboard_tokens) {
        g_ptr_array_free(state->clipboard_tokens, TRUE);
        state->clipboard_tokens = NULL;
    }

    g_free(state->clipboard_token);
    state->clipboard_token = g_strdup(token);
    if (token && *token) {
        state->clipboard_tokens = g_ptr_array_new_with_free_func(g_free);
        g_ptr_array_add(state->clipboard_tokens, g_strdup(token));
    }
    state->clipboard_cut = cut;
}

static GPtrArray *collect_operation_tokens(FilesState *state) {
    ...
    if (state->active_item_token && *state->active_item_token) {
        g_ptr_array_add(tokens, g_strdup(state->active_item_token));
    }

    return tokens;
}

static gboolean has_clipboard_content(FilesState *state) {
    return state
        && state->clipboard_tokens
        && state->clipboard_tokens->len > 0;
}

static void set_clipboard_from_selection(FilesState *state, gboolean cut) {
    GPtrArray *tokens = collect_operation_tokens(state);
    ...
    state->clipboard_tokens = tokens;

    g_free(state->clipboard_token);
    state->clipboard_token = g_strdup(g_ptr_array_index(state->clipboard_tokens, 0));
    state->clipboard_cut = cut;

    if (state->clipboard_tokens->len == 1) {
        gtk_label_set_text(
            GTK_LABEL(state->status_label),
            cut ? _("Cut to clipboard.") : _("Copied to clipboard.")
        );
    } else {
        char *msg = g_strdup_printf(
            cut
                ? ngettext("Cut %u item.", "Cut %u items.", state->clipboard_tokens->len)
                : ngettext("Copied %u item.", "Copied %u items.", state->clipboard_tokens->len),
            state->clipboard_tokens->len
        );
        gtk_label_set_text(GTK_LABEL(state->status_label), msg);
        g_free(msg);
    }
...
static gboolean spawn_command_with_error(char **argv, char **out_error_text) {
    if (!argv || !argv[0]) {
        return FALSE;
    }
    ...
    if (!spawned) {
        if (out_error_text) {
            *out_error_text = g_strdup(spawn_error ? spawn_error->message : _("Command execution failed."));
        }
    ...
    gboolean ok = (exit_status == 0);
    if (!ok && out_error_text) {
        if (stderr_text && *stderr_text) {
            char *trimmed = g_strdup(stderr_text);
            g_strstrip(trimmed);
            *out_error_text = (*trimmed) ? trimmed : g_strdup(_("Command failed."));
        } else {
            *out_error_text = g_strdup(_("Command failed."));
        }
    }
...
static gboolean extract_archive_token(FilesState *state, const char *archive_token) {
    if (!state || !archive_token || !is_local_path_token(archive_token)) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("Extract is supported only for local files."));
        return FALSE;
    }

    GFile *archive = g_file_new_for_path(archive_token);
    GFile *parent = g_file_get_parent(archive);
    char *archive_name = g_file_get_basename(archive);
    char *parent_path = parent ? g_file_get_path(parent) : NULL;
    char *lower = g_ascii_strdown(archive_name ? archive_name : "", -1);

    gboolean ok = FALSE;
    char *error_text = NULL;

    if (!parent_path || !archive_name || !*archive_name) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("Extract failed."));
        goto cleanup;
    }

    if (!token_looks_like_archive(archive_token)) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("Selected file is not a supported archive."));
        goto cleanup;
    }

    if (g_str_has_suffix(lower, ".zip")) {
        if (!g_find_program_in_path("unzip")) {
            gtk_label_set_text(GTK_LABEL(state->status_label), _("Missing tool: unzip."));
            goto cleanup;
        }

        char *argv[] = {"unzip", "-o", (char *)archive_token, "-d", parent_path, NULL};
        ok = spawn_command_with_error(argv, &error_text);
    } else if (g_str_has_suffix(lower, ".7z")) {
        if (!g_find_program_in_path("7z")) {
            gtk_label_set_text(GTK_LABEL(state->status_label), _("Missing tool: 7z."));
            goto cleanup;
        }

        char *out_dir = g_strdup_printf("-o%s", parent_path);
        char *argv[] = {"7z", "x", "-y", (char *)archive_token, out_dir, NULL};
        ok = spawn_command_with_error(argv, &error_text);
        g_free(out_dir);
    } else {
        if (!g_find_program_in_path("tar")) {
            gtk_label_set_text(GTK_LABEL(state->status_label), _("Missing tool: tar."));
            goto cleanup;
        }

        char *argv[] = {"tar", "-xf", (char *)archive_token, "-C", parent_path, NULL};
        ok = spawn_command_with_error(argv, &error_text);
    }

    if (!ok) {
        gtk_label_set_text(GTK_LABEL(state->status_label), error_text ? error_text : _("Extract failed."));
        goto cleanup;
    }

    gtk_label_set_text(GTK_LABEL(state->status_label), _("Archive extracted."));
...
static char *build_unique_archive_path(const char *dir_path, const char *name_base) {
    if (!dir_path || !*dir_path || !name_base || !*name_base) {
        return NULL;
    }

    char *candidate = g_strdup_printf("%s.tar.gz", name_base);
    char *full = g_build_filename(dir_path, candidate, NULL);
    guint idx = 1;

    while (g_file_test(full, G_FILE_TEST_EXISTS)) {
        g_free(candidate);
        g_free(full);
        candidate = g_strdup_printf("%s (%u).tar.gz", name_base, idx++);
        full = g_build_filename(dir_path, candidate, NULL);
    }
...
static gboolean compress_token_to_archive(FilesState *state, const char *item_token) {
    if (!state || !item_token || !is_local_path_token(item_token)) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("Archive creation is supported only for local files."));
        return FALSE;
    }

    GFile *item = g_file_new_for_path(item_token);
    GFile *parent = g_file_get_parent(item);
    char *item_name = g_file_get_basename(item);
    char *parent_path = parent ? g_file_get_path(parent) : NULL;

    if (!parent_path || !item_name || !*item_name) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("Archive creation failed."));
        g_free(parent_path);
        g_free(item_name);
        g_clear_object(&parent);
        g_object_unref(item);
    ...
    char *archive_path = build_unique_archive_path(parent_path, item_name);
    if (!archive_path) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("Archive creation failed."));
        g_free(parent_path);
        g_free(item_name);
    ...
    char *error_text = NULL;
    char *argv[] = {"tar", "-czf", archive_path, "-C", parent_path, item_name, NULL};
    gboolean ok = spawn_command_with_error(argv, &error_text);

    if (!ok) {
        gtk_label_set_text(GTK_LABEL(state->status_label), error_text ? error_text : _("Archive creation failed."));
        g_free(error_text);
        g_free(archive_path);
        g_free(parent_path);
        g_free(item_name);
    ...
    set_active_item(state, archive_path, FALSE);
    gtk_label_set_text(GTK_LABEL(state->status_label), _("Archive created."));
    refresh_current_view(state);
...
static char *query_hint_from_content_type(const char *content_type) {
    ...
    if (g_str_has_prefix(subtype, "x-")) {
        subtype += 2;
    }

    char *query = g_strdup(subtype);
...
static gboolean is_text_document_content_type(const char *content_type) {
    if (!content_type || !*content_type) {
        return FALSE;
    }
    ...
    if (g_str_has_suffix(content_type, "+xml") ||
        g_str_has_suffix(content_type, "+json") ||
        g_str_has_suffix(content_type, "+yaml") ||
        g_str_has_suffix(content_type, "+toml") ||
        g_str_has_suffix(content_type, "+csv")) {
        return TRUE;
    }

    if (g_content_type_is_a(content_type, "application/xml") ||
        g_content_type_is_a(content_type, "application/json") ||
        g_content_type_is_a(content_type, "application/ld+json") ||
        g_content_type_is_a(content_type, "application/javascript") ||
        g_content_type_is_a(content_type, "application/ecmascript") ||
        g_content_type_is_a(content_type, "application/x-shellscript") ||
        g_content_type_is_a(content_type, "application/x-python") ||
        g_content_type_is_a(content_type, "application/x-csrc") ||
        g_content_type_is_a(content_type, "application/x-c++src") ||
        g_content_type_is_a(content_type, "application/x-java") ||
        g_content_type_is_a(content_type, "application/x-php") ||
        g_content_type_is_a(content_type, "application/x-ruby") ||
        g_content_type_is_a(content_type, "application/x-perl") ||
        g_content_type_is_a(content_type, "application/x-lua") ||
        g_content_type_is_a(content_type, "application/vnd.oasis.opendocument.text") ||
        g_content_type_is_a(content_type, "application/vnd.oasis.opendocument.text-template") ||
        g_content_type_is_a(content_type, "application/vnd.oasis.opendocument.text-web") ||
        g_content_type_is_a(content_type, "application/vnd.openxmlformats-officedocument.wordprocessingml.document") ||
        g_content_type_is_a(content_type, "application/vnd.openxmlformats-officedocument.wordprocessingml.template") ||
        g_content_type_is_a(content_type, "application/vnd.ms-word.document.macroenabled.12") ||
        g_content_type_is_a(content_type, "application/vnd.ms-word.template.macroenabled.12") ||
        g_content_type_is_a(content_type, "application/rtf") ||
        g_content_type_is_a(content_type, "application/msword") ||
        g_content_type_is_a(content_type, "application/xhtml+xml")) {
        return TRUE;
    }
...
static char *query_hint_from_file(GFile *file) {
    if (!file) {
        return NULL;
    }
...
static gboolean launch_shop_for_mime(FilesState *state, GFile *file, const char *content_type, GError **error) {
    char *query = query_hint_from_file(file);
    if (!query || !*query) {
        g_free(query);
        query = query_hint_from_content_type(content_type);
    }

    char *command = NULL;
    if (query && *query) {
        char *quoted = g_shell_quote(query);
        command = g_strdup_printf("karton-shop --search %s", quoted);
        g_free(quoted);
    } else {
        command = g_strdup("karton-shop");
    }
    ...
    if (launched && state) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("Opened app store for this file type."));
    }

    return launched;
}

static void open_with_dialog_context_free(gpointer data, GClosure *closure) {
    (void)closure;
    OpenWithDialogContext *ctx = data;
    if (!ctx) {
        return;
    }
...
static void on_open_with_dialog_response(GtkDialog *dialog, int response, gpointer user_data) {
    OpenWithDialogContext *ctx = user_data;
    if (!ctx || !ctx->state) {
    ...
    if (response == GTK_RESPONSE_ACCEPT) {
        GAppInfo *app = gtk_app_chooser_get_app_info(GTK_APP_CHOOSER(ctx->chooser));
        if (!app) {
            gtk_label_set_text(GTK_LABEL(ctx->state->status_label), _("Select an application first."));
            gtk_window_destroy(GTK_WINDOW(dialog));
            return;
        }

        GList *files = NULL;
        files = g_list_append(files, ctx->file);
        ...
        if (!launched) {
            gtk_label_set_text(
                ...
                error ? error->message : _("Could not open selected application.")
            );
        } else {
            gtk_label_set_text(GTK_LABEL(ctx->state->status_label), _("Opened with selected application."));
        }
        g_clear_error(&error);
    } else if (response == GTK_RESPONSE_APPLY) {
        GError *error = NULL;
        if (!launch_shop_for_mime(ctx->state, ctx->file, ctx->content_type, &error)) {
            gtk_label_set_text(
                GTK_LABEL(ctx->state->status_label),
                error ? error->message : _("Could not start app store.")
            );
            g_clear_error(&error);
        }
    }
...
static void show_open_with_dialog(FilesState *state, GFile *file, const char *content_type) {
    if (!state || !file || !content_type || !*content_type) {
        gtk_label_set_text(GTK_LABEL(state->status_label), _("No application is associated with this file type."));
        return;
    }
    ...
    GtkWidget *dialog = gtk_dialog_new_with_buttons(
        ...
        _("Install app"), GTK_RESPONSE_APPLY,
    ...
    char *type_desc = g_content_type_get_description(content_type);
    gboolean use_text_fallback = is_text_document_content_type(content_type) && !g_content_type_is_a(content_type, "text");
    const char *chooser_type = use_text_fallback ? "text/plain" : content_type;
    char *hint = NULL;

    if (use_text_fallback) {
        hint = g_strdup_printf(_("No default app was found. Choose an installed app or text editor for: %s"), type_desc ? type_desc : content_type);
    } else {
        hint = g_strdup_printf(_("No default app was found. Choose an installed app for type: %s"), type_desc ? type_desc : content_type);
    }

    gtk_label_set_text(GTK_LABEL(description), hint);
    gtk_label_set_wrap(GTK_LABEL(description), TRUE);
    gtk_widget_set_halign(description, GTK_ALIGN_START);
    gtk_widget_set_margin_start(description, 12);
    gtk_widget_set_margin_end(description, 12);
    gtk_widget_set_margin_top(description, 12);
    gtk_widget_set_margin_bottom(description, 8);
    gtk_box_append(GTK_BOX(content), description);

    GtkWidget *chooser = gtk_app_chooser_widget_new(chooser_type);
    ...
    gtk_app_chooser_widget_set_show_fallback(GTK_APP_CHOOSER_WIDGET(chooser), TRUE);
    gtk_app_chooser_widget_set_show_other(GTK_APP_CHOOSER_WIDGET(chooser), TRUE);
    gtk_widget_set_margin_start(chooser, 12);
    gtk_widget_set_margin_end(chooser, 12);
    gtk_widget_set_margin_bottom(chooser, 12);
    gtk_widget_set_vexpand(chooser, TRUE);
    gtk_box_append(GTK_BOX(content), chooser);

    OpenWithDialogContext *ctx = g_new0(OpenWithDialogContext, 1);
    ctx->state = state;
    ctx->file = g_object_ref(file);
    ctx->chooser = chooser;
    ctx->content_type = g_strdup(content_type);

    g_signal_connect_data(dialog, "response", G_CALLBACK(on_open_with_dialog_response), ctx, open_with_dialog_context_free, 0);
...
static gboolean open_file_with_association(FilesState *state, const char *token) {
    ...
    if (!launched) {
        if (content_type) {
            show_open_with_dialog(state, file, content_type);
        } else {
            gtk_label_set_text(
                GTK_LABEL(state->status_label),
...
static void copy_active_item(FilesState *state) {
    set_clipboard_from_selection(state, FALSE);
}

static void cut_active_item(FilesState *state) {
    set_clipboard_from_selection(state, TRUE);
}
...
static gboolean move_active_to_trash(FilesState *state) {
    ...
    for (guint i = 0; i < tokens->len; i++) {
        ...
        if (!file) {
            if (!first_error) {
                first_error = g_strdup(_("Delete failed."));
            }
            continue;
        }
        ...
        if (!ok) {
            if (!first_error) {
                first_error = g_strdup(error ? error->message : _("Delete failed."));
            }
            g_clear_error(&error);
            g_object_unref(file);
            continue;
    ...
    if (deleted == 0) {
        gtk_label_set_text(GTK_LABEL(state->status_label), first_error ? first_error : _("Delete failed."));
        g_free(first_error);
        return FALSE;
    }
    ...
    if (deleted == 1) {
        ...
        char *msg = g_strdup_printf(
            is_trash_token(state->current_token)
                ? ngettext("Deleted %u item from trash.", "Deleted %u items from trash.", deleted)
                : (!state->feature_trash
                    ? ngettext("Deleted %u item permanently.", "Deleted %u items permanently.", deleted)
                    : ngettext("Moved %u item to trash.", "Moved %u items to trash.", deleted)),
            deleted
        );
        gtk_label_set_text(GTK_LABEL(state->status_label), msg);
        g_free(msg);
...
static gboolean paste_clipboard_to_current(FilesState *state) {
    if (!has_clipboard_content(state)) {
    ...
    for (guint i = 0; i < state->clipboard_tokens->len; i++) {
        const char *clipboard_token = g_ptr_array_index(state->clipboard_tokens, i);
        GFile *src = file_from_token(clipboard_token);
        if (!src) {
            if (!first_error) {
                first_error = g_strdup(_("Paste failed."));
            }
            continue;
        }
        ...
        if (g_file_equal(src, dst)) {
            if (!first_error) {
                first_error = g_strdup(_("Source and destination are the same."));
            }
            g_free(basename);
            g_free(src_token);
            g_free(dst_token);
    ...
    if (pasted == 1) {
        ...
        char *msg = g_strdup_printf(ngettext("Pasted %u item.", "Pasted %u items.", pasted), pasted);
        gtk_label_set_text(GTK_LABEL(state->status_label), msg);
        g_free(msg);
    }
...
static void on_menu_trash_clicked(GtkButton *button, gpointer user_data) {
    ...
    gtk_popover_popdown(GTK_POPOVER(popover));
    set_active_item(ctx->state, ctx->item_token, ctx->item_is_dir);
    move_active_to_trash(ctx->state);
}

static void on_menu_extract_clicked(GtkButton *button, gpointer user_data) {
    (void)button;
    GtkWidget *popover = GTK_WIDGET(user_data);
    TileMenuContext *ctx = popover_menu_context(popover);
    if (!ctx) {
    ...
    gtk_popover_popdown(GTK_POPOVER(popover));
    set_active_item(ctx->state, ctx->item_token, ctx->item_is_dir);
    extract_archive_token(ctx->state, ctx->item_token);
}

static void on_menu_compress_clicked(GtkButton *button, gpointer user_data) {
    (void)button;
    GtkWidget *popover = GTK_WIDGET(user_data);
    TileMenuContext *ctx = popover_menu_context(popover);
    if (!ctx) {
    ...
    gtk_popover_popdown(GTK_POPOVER(popover));
    set_active_item(ctx->state, ctx->item_token, ctx->item_is_dir);
    compress_token_to_archive(ctx->state, ctx->item_token);
}
...
static void setup_tile_context_menu(GtkWidget *tile_button, FilesState *state, const char *item_token, gboolean item_is_dir) {
    ...
    GtkWidget *extract_btn = gtk_button_new_with_label(_("Extract here"));
    GtkWidget *compress_btn = gtk_button_new_with_label(_("Compress to archive"));
    ...
    gtk_widget_add_css_class(extract_btn, "flat-button");
    gtk_widget_add_css_class(compress_btn, "flat-button");
    ...
    gtk_widget_add_css_class(extract_btn, "context-menu-item");
    gtk_widget_add_css_class(compress_btn, "context-menu-item");
    ...
    gtk_widget_set_visible(extract_btn, !item_is_dir && token_looks_like_archive(item_token));
    gtk_widget_set_visible(compress_btn, is_local_path_token(item_token));
    ...
    g_signal_connect(extract_btn, "clicked", G_CALLBACK(on_menu_extract_clicked), popover);
    g_signal_connect(compress_btn, "clicked", G_CALLBACK(on_menu_compress_clicked), popover);
    ...
    gtk_box_append(GTK_BOX(box), extract_btn);
    gtk_box_append(GTK_BOX(box), compress_btn);
...
static GtkWidget *create_file_tile(FilesState *state, const FileItem *item) {
    ...
    gboolean dense_grid = !state->list_view
        && state->flowbox
        && gtk_widget_has_css_class(state->flowbox, "files-grid-dense");
    int tile_icon_size = state->list_view ? 20 : (dense_grid ? MAX(32, state->icon_size - 18) : state->icon_size);
    gtk_picture_set_content_fit(GTK_PICTURE(icon), GTK_CONTENT_FIT_CONTAIN);
    gtk_picture_set_can_shrink(GTK_PICTURE(icon), TRUE);
    ...
    gtk_label_set_max_width_chars(GTK_LABEL(label), state->list_view ? 64 : (dense_grid ? 12 : 16));
    ...
    if (state->list_view) {
        ...
        if (dense_grid) {
            gtk_widget_set_size_request(button, tile_icon_size + 34, tile_icon_size + 34);
        } else {
...
static void populate_items(FilesState *state, GPtrArray *items) {
    ...
    apply_grid_density_for_item_count(state, items ? items->len : 0);
    clear_flowbox(state);
    for (guint i = 0; i < items->len; i++) {
        ...
        GtkWidget *flowbox_child = gtk_widget_get_parent(tile);
        if (flowbox_child && GTK_IS_FLOW_BOX_CHILD(flowbox_child)) {
            gtk_widget_set_halign(flowbox_child, GTK_ALIGN_FILL);
            gtk_widget_set_valign(flowbox_child, GTK_ALIGN_FILL);
            gtk_widget_set_hexpand(flowbox_child, state->list_view);
        }
    }
...
static gboolean load_from_token(FilesState *state, const char *token) {
    ...
    char *previous_token = state->current_token;
    ...
    if (!ok) {
        if (changed_location) {
            ...
            state->current_token = previous_token;
        }
        return FALSE;
    }
...
static void apply_view_mode(FilesState *state) {
    ...
    if (state->list_view) {
        gtk_widget_remove_css_class(state->flowbox, "files-grid-dense");
...
static void apply_grid_density_for_item_count(FilesState *state, guint item_count) {
    if (!state || !state->flowbox || state->list_view) {
        return;
    }

    gboolean dense = item_count >= 120;
    if (dense) {
        gtk_widget_add_css_class(state->flowbox, "files-grid-dense");
        gtk_flow_box_set_homogeneous(GTK_FLOW_BOX(state->flowbox), TRUE);
        gtk_flow_box_set_min_children_per_line(GTK_FLOW_BOX(state->flowbox), 6);
        gtk_flow_box_set_max_children_per_line(GTK_FLOW_BOX(state->flowbox), 18);
        gtk_flow_box_set_row_spacing(GTK_FLOW_BOX(state->flowbox), 8);
        gtk_flow_box_set_column_spacing(GTK_FLOW_BOX(state->flowbox), 8);
    } else {
        gtk_widget_remove_css_class(state->flowbox, "files-grid-dense");
        gtk_flow_box_set_homogeneous(GTK_FLOW_BOX(state->flowbox), TRUE);
        gtk_flow_box_set_min_children_per_line(GTK_FLOW_BOX(state->flowbox), 4);
...
GtkWidget *karton_files_window_new(GtkApplication *app, const char *start_token) {
    ...
    rebuild_sidebar(state);
    apply_view_mode(state);
    navigate_to_token(state,
        (start_token && *start_token) ? start_token : g_get_home_dir(),
        TRUE);
