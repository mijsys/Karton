...
static GtkWidget *open_files_window(GtkApplication *app, const char *start_token);
...
static void recreate_main_window(void) {
    ...
    g_main_window = karton_files_window_new(app, NULL);
    apply_runtime_theme_mode();
    gtk_window_present(GTK_WINDOW(g_main_window));
}

static GtkWidget *open_files_window(GtkApplication *app, const char *start_token) {
    GtkWidget *window = karton_files_window_new(app, start_token);
...
static void on_activate(GtkApplication *app, gpointer user_data) {
    ...
    load_app_css();
    GtkWidget *window = open_files_window(app, NULL);
...
static int on_command_line(GApplication *application, GApplicationCommandLine *command_line, gpointer user_data) {
    ...
    char **argv = g_application_command_line_get_arguments(command_line, NULL);
    ...
    open_files_window(app, start_token);
    g_strfreev(argv);
    return 0;
}
...
int main(int argc, char **argv) {
    ...
    GtkApplication *app = gtk_application_new("io.karton.Files", G_APPLICATION_HANDLES_COMMAND_LINE);
