 ...
 * pairs in `${XDG_CONFIG_DIRS:-/etc/xdg}/karton/environment` with user override
 ...
 * Note: Same as `sh ~/.config/karton/autostart` (or equivalent XDG config dir)
 */
void session_autostart_init(void);

/**
 * session_shutdown - run session shutdown file as shell script
 * Note: Same as `sh ~/.config/karton/shutdown` (or equivalent XDG config dir)
 */
