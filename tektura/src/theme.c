 ...
 *
 * theme_builtin() applies a bold built-in fallback theme
 */
static void
theme_builtin(struct theme *theme)
{
	theme->border_width = 2;
	theme->window_titlebar_padding_height = 4;
	theme->window_titlebar_padding_width = 8;
	theme->window_title_separator_width = 2;
	parse_hexstr("#f4b400", theme->window_title_separator_color[SSD_ACTIVE]);
	parse_hexstr("#6c757d", theme->window_title_separator_color[SSD_INACTIVE]);

	parse_hexstr("#2d6a4f", theme->window[SSD_ACTIVE].border_color);
	parse_hexstr("#6c757d", theme->window[SSD_INACTIVE].border_color);

	parse_hexstr("#ff7a18", theme->window_toggled_keybinds_color);

	theme->window[SSD_ACTIVE].title_bg.gradient = LAB_GRADIENT_VERTICAL;
	theme->window[SSD_INACTIVE].title_bg.gradient = LAB_GRADIENT_VERTICAL;
	parse_hexstr("#1f3d38", theme->window[SSD_ACTIVE].title_bg.color);
	parse_hexstr("#305c53", theme->window[SSD_ACTIVE].title_bg.color_to);
	parse_hexstr("#2c2f33", theme->window[SSD_INACTIVE].title_bg.color);
	parse_hexstr("#3a3f45", theme->window[SSD_INACTIVE].title_bg.color_to);
	...
	parse_hexstr("#f6f5ec", theme->window[SSD_ACTIVE].label_text_color);
	parse_hexstr("#d5d3c8", theme->window[SSD_INACTIVE].label_text_color);
	theme->window_label_text_justify = parse_justification("Left");

	theme->window_button_width = 30;
	theme->window_button_height = 24;
	theme->window_button_spacing = 6;

	parse_hexstr("#ffffff1f", theme->window_button_hover_bg_color);
	theme->window_button_hover_bg_corner_radius = 6;
	...
	theme->window[SSD_ACTIVE].shadow_size = 72;
	theme->window[SSD_INACTIVE].shadow_size = 52;
	parse_hexstr("#00000080", theme->window[SSD_ACTIVE].shadow_color);
	parse_hexstr("#00000055", theme->window[SSD_INACTIVE].shadow_color);
...
entry(struct theme *theme, const char *key, const char *value)
{
	...
	if (match_glob(key, "window.title.separator.width")) {
		theme->window_title_separator_width = get_int_if_positive(
			value, "window.title.separator.width");
	}
	if (match_glob(key, "window.active.title.separator.color")) {
		parse_color(value, theme->window_title_separator_color[SSD_ACTIVE]);
	}
	if (match_glob(key, "window.inactive.title.separator.color")) {
		parse_color(value, theme->window_title_separator_color[SSD_INACTIVE]);
	}
...
post_processing(struct theme *theme)
{
	...
	if (theme->window_title_separator_width > theme->titlebar_height) {
		wlr_log(WLR_ERROR,
			"window.title.separator.width (%d) exceeds titlebar height (%d), clamping.",
			theme->window_title_separator_width, theme->titlebar_height);
		theme->window_title_separator_width = theme->titlebar_height;
	}
...
theme_init(struct theme *theme, const char *theme_name)
{
	...
	if (theme_name) {
		/*
		 * Read
		 *   - <data-dir>/share/themes/$theme_name/tekstura/themerc
	...
	/* Read <config-dir>/tekstura/themerc-override */
