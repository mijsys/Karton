...
static void
build_theme_path_tekstura(struct ctx *ctx, char *prefix, const char *path)
{
	assert(prefix);
	snprintf(ctx->buf, ctx->len, "%s/%s/%s/tekstura/%s", prefix, path,
		...
		const char *filename)
{
	...
	wl_list_init(paths);
	struct ctx ctx = {
		.build_path_fn = build_theme_path_tekstura,
		.filename = filename,
		.buf = buf,
		.len = sizeof(buf),
		.dirs = theme_dirs,
