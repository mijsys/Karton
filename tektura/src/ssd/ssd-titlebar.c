...
ssd_titlebar_create(struct ssd *ssd)
{
	...
	FOR_EACH_ACTIVE_STATE(active) {
		...
		subtree->separator = lab_wlr_scene_rect_create(parent,
			...
			theme->window_title_separator_width,
			theme->window_title_separator_color[active]);
		wlr_scene_node_set_position(&subtree->separator->node,
			corner_width,
			theme->titlebar_height - theme->window_title_separator_width);
		wlr_scene_node_set_enabled(&subtree->separator->node,
			theme->window_title_separator_width > 0);
...
set_squared_corners(struct ssd *ssd, bool enable)
{
	...
	FOR_EACH_ACTIVE_STATE(active) {
		struct ssd_titlebar_subtree *subtree = &ssd->titlebar.subtrees[active];
		int bar_width = MAX(width - 2 * x, 0);

		wlr_scene_node_set_position(&subtree->bar->node, x, 0);
		wlr_scene_buffer_set_dest_size(subtree->bar,
			bar_width, theme->titlebar_height);
		wlr_scene_rect_set_size(subtree->separator,
			bar_width, theme->window_title_separator_width);
		wlr_scene_node_set_position(&subtree->separator->node,
			x, theme->titlebar_height - theme->window_title_separator_width);
		wlr_scene_node_set_enabled(&subtree->separator->node,
			theme->window_title_separator_width > 0);
...
ssd_titlebar_update(struct ssd *ssd)
{
	...
	FOR_EACH_ACTIVE_STATE(active) {
		struct ssd_titlebar_subtree *subtree = &ssd->titlebar.subtrees[active];
		int bar_width = MAX(width - bg_offset * 2, 0);
		wlr_scene_buffer_set_dest_size(subtree->bar,
			bar_width, theme->titlebar_height);
		wlr_scene_rect_set_size(subtree->separator,
			bar_width, theme->window_title_separator_width);
